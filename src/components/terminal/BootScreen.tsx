...
const bootSequence: BootLine[] = [
  { text: 'kiover@portfolio:~$ npm run kiover-portfolio', delay: 0, color: 'text-terminal-green' },
  { text: '', delay: 400 },
  { text: '> kiover-portfolio@1.0.0 start', delay: 600, color: 'text-terminal-muted' },
  { text: '> vite --host', delay: 800, color: 'text-terminal-muted' },
  { text: '', delay: 1000 },
  { text: '  VITE v7.2.4  ready in 420ms', delay: 1200, color: 'text-terminal-green' },
  { text: '', delay: 1400 },
  { text: '  ➜  Local:   http://localhost:5173/', delay: 1600, color: 'text-terminal-blue' },
...
export function BootScreen({ onComplete }: BootScreenProps) {
  ...
  return (
    <AnimatePresence>
      {!exiting && (
        <motion.div
          className="fixed inset-0 z-[9999] bg-terminal-bg flex flex-col justify-center p-6 md:p-12 scanlines cursor-pointer"
          exit={{ opacity: 0 }}
        ...
        >
          <div className="max-w-2xl w-full mx-auto font-mono text-sm md:text-base">
            ...
            {phase === 'loading' && (
              <div className="mt-2">
                <span className="text-terminal-amber">{progressBar()}</span>
              </div>
            )}
            ...
            {phase === 'done' && (
              <motion.div
                ...
                className="mt-2"
              >
                <span className="text-terminal-green terminal-glow-strong">✓ Portfolio loaded successfully</span>
              </motion.div>
            )}
          </div>
